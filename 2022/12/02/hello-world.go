...
func main() {
	...
	locAlist := getLocationAList(wordBreakDown, 97)
	maxResult := 9999999999999
	for _, startloc := range locAlist {
		// fmt.Printf("%v\n", rowLength)
		// fmt.Printf("%v\n", startloc)
		// fmt.Printf("%v\n", endIndex)
		...
		for steps := 0; stepsBreakDown[endIndex[0]][endIndex[1]] == 0 && steps < 500; steps++ {
			// for steps := 1; steps < 30; steps++ {
			locationList := [][]int{}
			if steps == 1 {
				locationList = append(locationList, startloc)
			} else {
		...
		result := stepsBreakDown[endIndex[0]][endIndex[1]]
		// fmt.Printf("From start %v the result is %v.\n", startloc, result)
		if result < maxResult && result != 0 {
			maxResult = result
		}
	}

	fmt.Printf("The max steps with a is %v\n", maxResult)
}

func getLocationAList(steps [][]rune, char rune) [][]int {
	locList := [][]int{}
	for indexA, element := range steps {
		for index := range element {
			// fmt.Printf("%v\n", steps[indexA][index])
			if steps[indexA][index] == char {
