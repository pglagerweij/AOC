...
func main() {
	...
	if err != nil {
		fmt.Printf("Could notc read the file due to this %s error \n", err)
	}
	...
	for roundNumber := 1; roundNumber <= 10000; roundNumber++ {
		for _, element := range s3 {
			...
			for _, currentVal := range currentValues[monkeyIndex] {
				...
				newNumber := performOperation(instructions, currentVal, 1)
	...
	fmt.Printf("total assesments is %v.\n", totalAssessment)
...
func determineResult(number int, divnumber int) (bool, int) {
	if number%divnumber == 0 {
		return true, number % 145495350
	} else {
		return false, number % 145495350
	}
}
...
func performOperation(operation string, inputNumber int, divisionNumber int) int {
	if operation == "new = old * 3" {
	...
	} else if operation == "new = old * 19" {
		return inputNumber * 19
		// return int(math.Floor(float64((inputNumber * 19)) / float64(divisionNumber)))
		...
		// return int(math.Floor(float64((inputNumber + 6)) / float64(divisionNumber)))
