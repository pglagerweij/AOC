...
func main() {
	...
	for ind, s := range strings.Split(strings.TrimSpace(string(input)), "\n") {
		...
		inputSlice[ind] = totalSlice{ind, sint * decryptionKey}
		iterationSlice = append(iterationSlice, totalSlice{ind, sint * decryptionKey})
	...
	for index, adjustedElem := range iterationSlice {
		if inputelem == adjustedElem {
			...
			if newIndex == 0 {
				...
				iterationSlice = insert(RemoveIndex(iterationSlice, index), 0, adjustedElem)
